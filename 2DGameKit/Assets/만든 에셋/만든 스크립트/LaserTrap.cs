...
public class LaserTrap : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (damager == null && damage > 0)
        {
            ...
            damager.offset = new Vector2(laserLength / 2f, 0f);
            damager.size = new Vector2(laserLength, laserWidth);
    ...
    void UpdateLaserCollider()
    {
        ...
        // 콜라이더를 레이저 중심에 배치
        laserCollider.offset = new Vector2(currentLaserLength / 2f, 0f);
        laserCollider.size = new Vector2(currentLaserLength, laserWidth);

        // Damager의 offset과 size도 업데이트
        if (damager != null)
        {
            damager.offset = new Vector2(currentLaserLength / 2f, 0f);
            damager.size = new Vector2(currentLaserLength, laserWidth);
        }
    }
